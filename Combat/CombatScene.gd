...
var healthyBar = "res://Combat/ui/HealthyHealthProgress.png"
var cautionBar = "res://Combat/ui/CautionHealthProgress.png"
var dangerBar = "res://Combat/ui/DangerHealthProgress.png"
...
func wild_combat_start():
	...
	playerSprite1.texture = load("res://PokemonSprites/"+playerActive1.pokemon.pokedexInfo.get("ID")+".png")
	playerMoves = playerActive1.pokemon.moves
	
	enemySprite1.texture = load("res://PokemonSprites/"+enemyActive1.pokemon.pokedexInfo.get("ID")+".png")
...
func buttonMoves():
	...
	for i in numMoves:
		...
		buttons["Move"+str(i+1)].texture_normal = load("res://Combat/ui/buttons/"+buttonType+"_Button.png")
...
func Outcome(playerSelectedMove):
	...
	# speedCalculator(moves) this doesn't work
	...
	while finishedTextLoop == false:
		...
		if(str(moves[0]) == "notEffective" or str(moves[0]) == "effective" or str(moves[0]) == " "):
			otherText(moves[0])
		else:
			var moveHit = doesMoveHit(moves[0])
			moveText(moveHit, moves[0])
			category = moves[0].attack.Category
		if(str(moves[0]) == "notEffective" or str(moves[0]) == "effective"):
			pass
		...
		if(1 == len(moves)):
			...
			if(str(moves[1]) != "notEffective" and str(moves[1]) != "effective"):
				if(playerActive1.pokemon.tempHp == 0):
					combatEnd(enemyActive1)
					moves.insert(1, " ")
				elif(enemyActive1.pokemon.tempHp == 0):
...
func speedCalculator(unsortedMoves):
	...
	# Woah, a really bad selection sort algorithim!
	# sorts based off of speed
	# I need to add another sort before this that
	# sorts and removes stuff based off of priority, not speed
	# + priority just pops it to the front, - priority will add a
	# list after the speed sort so its always at the end
	...
	for i in range(moves):
		...
		for j in range(moves):
			if(minValue == null):
				minValue = moves[i]
			elif(minValue.speed > moves[i].speed):
				minValue = moves[i]
			elif(minValue.speed == moves[i].speed):
